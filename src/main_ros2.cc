...
public:
  ...
  { 
    ...
    sub_node_control = this->create_subscription<std_msgs::msg::String>("/node_control",10,std::bind(&HesaiLidarClient::node_control_callback,this,std::placeholders::_1));//lz                          
...
private:
  ...
  void lidarCallback(boost::shared_ptr<PPointCloud> cld, double timestamp, hesai_lidar::msg::PandarScan::SharedPtr scan) // the timestamp from first point cloud of cld
  {
    if(m_sPublishType == "both" || m_sPublishType == "points"){
      ...
      // cld->header.stamp=timestamp;
      sensor_msgs::msg::PointCloud2 output;
      pcl::toROSMsg(*cld, output);
      output.header.stamp.sec = int(timestamp);
      output.header.stamp.nanosec=(timestamp-output.header.stamp.sec)*1e9;
      // output.header.stamp.nanosec = timestamp;
      //<<setprecision(18)
      //std::cout<<fixed<<"output_time:"<<output.header.stamp.nanosec<<endl;
...
//       // std::cout << "cld->header.stamp: " << std::setprecision(10) << cld->header.stamp << ", point size: " << cld->points.size() << std::endl;
//       std::cout << "output.header.stamp.sec: " << std::setprecision(10) << output.header.stamp.sec << ", point size: " << cld->points.size() << std::endl;
//  std::cout << "output.header.stamp.nanosec: " << std::setprecision(10) << output.header.stamp.nanosec << ", point size: " << cld->points.size() << std::endl;
#ifdef PRINT_FLAG
        std::cout.setf(ios::fixed);
        std::cout << "timestamp: " << std::setprecision(10) << timestamp << ", point size: " << cld->points.size() << std::endl;
#endif        
  ...
  void timer_callback()
  {
    ...
    RCLCPP_INFO(this->get_logger(),"ip:%s",serverIp.c_str());
